...
int CommService::init(const struct sockaddr* bind_addr, socklen_t addrlen, int listen_timeout, int response_timeout)
{
    int ret;
...
void Communicator::handle_listen_result(struct poller_result* res)
{
    ...
    case PR_ST_SUCCESS:
        ...
        if (entry)
        {
            if (service->ssl_ctx)
            {
                if (__create_ssl(service->ssl_ctx, entry) >= 0 && service->init_ssl(entry->ssl) >= 0)
                {
